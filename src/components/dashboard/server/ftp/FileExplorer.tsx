...
export default function FileExplorer() {
  ...
  const handleDelete = () => {
    setFiles((prevFiles) =>
      prevFiles.filter((file) => !selectedItems.includes(file.id))
    );
    setSelectedItems([]);
  };
  ...
  const handleRename = () => {
    if (newName.trim() && selectedItems.length === 1) {
      setFiles((prevFiles) =>
        prevFiles.map((file) =>
          file.id === selectedItems[0] ? { ...file, name: newName } : file
        )
      );
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="flex items-center mb-4 gap-2">
        <Button
          ...
          disabled={pathHistory.past.length === 0}
        >
        ...
        <Button
          ...
          disabled={pathHistory.future.length === 0}
        >
          ...
          <BreadcrumbList>
            {pathHistory.present.map((item, index) => (
              <React.Fragment key={index}>
                ...
                {index < pathHistory.present.length - 1 && (
                  <BreadcrumbSeparator />
                )}
              </React.Fragment>
            ))}
            ...
            <TableRow key={file.id}>
              <TableCell>
                <Checkbox
                  ...
                  onCheckedChange={() => handleItemSelect(file.id)}
                />
              </TableCell>
